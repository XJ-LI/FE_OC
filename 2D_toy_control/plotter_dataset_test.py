...
for name, mat_path, idx, n_cases in tests:
    ...
    query_indices = torch.randint(0, 40*20, (500,), device=device)
    example_xs = xs.reshape(n_cases, -1, 3)[idx, query_indices, :].reshape(1, 500, 3)
    example_ys = ys.reshape(n_cases, -1, 2)[idx, query_indices, :].reshape(1, 500, 2)
    ...
    plt.tight_layout()
    plt.savefig(f"oc_test_{name}.png", bbox_inches='tight', dpi=400)
    plt.close()
    ...
    for i in range(n_cases):
        ...
        query_indices = torch.randint(0, 40*20, (500,), device=device)
        example_xs = xs.reshape(n_cases, -1, 3)[i, query_indices, :].reshape(1, 500, 3)
        example_ys = ys.reshape(n_cases, -1, 2)[i, query_indices, :].reshape(1, 500, 2)
        representation, _ = model.compute_representation(example_xs, example_ys, method="least_squares")

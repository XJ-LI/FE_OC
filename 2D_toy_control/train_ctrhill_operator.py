...
data = loadmat(f'{logdir}/Dataset/oc_velocity_dataset_B2B.mat')
target_states = torch.tensor(data['target_states']).float().to(device)
trajectories = torch.tensor(data['positions_times'].reshape(256, 200, 3)).float().to(device)
controls = torch.tensor(data['velocities'].reshape(256, 200, 2)).float().to(device)

data = loadmat(f'{logdir}/Dataset/oc_velocity_dataset_unseen1.mat')
target_states_val = torch.tensor(data['target_states']).float().to(device)
trajectories_val = torch.tensor(data['positions_times'].reshape(5, -1, 3)).float().to(device)
controls_val = torch.tensor(data['velocities'].reshape(5, -1, 2)).float().to(device)

# load the pre-traiend FE model
FE_model = FunctionEncoder(input_size=(3,),
                        output_size=(2,),
...
if __name__ == "__main__":
    ...
    model = MLP(input_dim=2, hidden_dim=256, output_dim=n_basis).to(device)
    ...
    for epoch in range(epochs+1):
        # get data
        function_indices = torch.randint(0, 256, (batch_size,), device=device)
        example_xs = trajectories[function_indices]
        example_ys = controls[function_indices]
        target_loc = target_states[function_indices,:]
        
        xs = trajectories_val
        ys = controls_val
        target_loc_val = target_states_val
